...
class SaleOrder(models.Model):
    ...
    # --- HR outsourcing: bill the client based on the salaries batch paid for them ---
    is_salaries = fields.Boolean(
        string="Is Salaries?",
        help="Check this when this Sales Order is how we invoice a client company "
             "for the HR/payroll service we run for them (we take our fee/commission "
             "on the salaries batch we processed on their behalf).",
    )
    payslip_run_id = fields.Many2one(
        'hr.payslip.run',
        string="Salaries Batch",
        # NOTE: adjust the domain state value below to whatever your final
        # "closed/paid" state of hr.payslip.run is actually called.
        domain="[('state', '=', 'close')]",
        help="Select the Payslips Batch that was already disbursed, so this "
             "Sales Order/Invoice reflects the salaries run we are billing "
             "our HR service fee/commission on.",
    )
